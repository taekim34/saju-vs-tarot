...
## 사주 구조 요약
일간, 오행 비율, 핵심 십성을 3~4줄로 요약

## 이 질문에 대한 사주 분석
사주 구조가 이 고민/질문과 어떻게 연결되는지 5~8줄로 설명
- 구체적인 사주 요소(십성, 오행, 충/합 등)를 근거로 제시
- "왜 이런 상황이 생기는지"를 사주 구조로 설명

## 사주가 알려주는 해결 방향
3~5줄의 구체적 조언 (사주 근거 + 실전 행동)

## ${currentYear}년 운의 흐름
세운과 대운이 이 문제에 미치는 영향 2~3줄` :

`# ${topicEmoji[topic]} ${topic} 분석

## 사주 핵심 구조
일간 특성, 오행 비율, 핵심 십성을 3~4줄로 요약

## ${topic}에 영향을 주는 사주 요소
관련 십성, 오행, 충/합, 12운성 등을 근거로 5~8줄 해석
- 좋은 요소와 주의할 요소를 구분
- 각 요소가 실생활에서 어떻게 나타나는지 설명

## ${currentYear}년 ${topic} 흐름
세운과 대운이 올해 미치는 영향 3~4줄

## 실전 조언
구체적이고 실행 가능한 조언 2~3줄`}

## 💫 핵심 한 줄
인상적인 마무리 메시지 (✨로 시작)

800~1200자로 답변하세요. 마크다운 문법(#, ##, **, -, 이모지)을 적극 활용하세요.`;
...
성별: ${gender === 'male' ? '남성' : '여성'}`;
    ...
    return callAPI(systemPrompt, userPrompt, 2, 0, 4000);
  }
...
## 카드 에너지 요약
메이저/마이너 비율, 전체 에너지 톤 2~3줄

## 카드별 이야기
각 카드의 의미를 위치(과거→현재→미래)와 연결하여 설명
- 카드 이미지를 생생하게 묘사
- 각 카드가 이 질문에 대해 무엇을 말하는지

## 이 질문에 대한 카드의 답
카드들이 만들어내는 스토리로 질문에 답 5~8줄
...
각 카드의 의미를 위치(과거→현재→미래)와 연결하여 설명
- 카드 이미지를 생생하게 묘사
- 각 카드가 ${topic}에 대해 무엇을 말하는지

## 카드들의 대화 — 숨겨진 메시지
카드 간 관계, 수트/숫자 패턴이 만들어내는 서사 3~5줄
